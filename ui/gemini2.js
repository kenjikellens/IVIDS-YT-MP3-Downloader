...
/* Model fallback priority list (matches DO_NOT_EDIT/gemini2.py) */
const MODELS = [
    "gemini-2.5-flash",
...
/**
 * Queries Gemini with API Key 2 via sequential model fallback.
...
async function ask_ai(filename, folder_name, tags) {
    ...
    for (let modelName of MODELS) {
        try {
            console.log(`[Gemini 2] Proberen met model: ${modelName}`);
            const config = {
            ...
            lastError = err;
            console.warn(`[Gemini 2] Failed with model ${modelName}: ${err.message}`);
