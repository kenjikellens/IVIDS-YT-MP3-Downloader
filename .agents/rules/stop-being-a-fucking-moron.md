...
6. **BUILD OUTPUT DIRECTORY**: Ensure that any built executable files (`.exe`, `.apk`, etc.) are always moved or generated in the root directory of the project, and the `dist` and `build` directories are removed/deleted afterwards.
